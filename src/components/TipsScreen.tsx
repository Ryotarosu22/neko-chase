...
import { TIP_CATEGORIES, findCategory, findArticle } from '../tips';
...
export default function TipsScreen({ onBack }: Props) {
  ...
  if (article && category) {
    return (
      <div className="flex flex-col h-full max-w-lg mx-auto bg-white">
        <Header
          title={category.name}
          onBack={() => setArtId(null)}
        />
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <p className="text-xs text-gray-400 mb-1">{category.icon} {category.name}</p>
          <h1 className="text-xl font-bold text-gray-800 mb-4 leading-snug">{article.title}</h1>
  ...
  if (category) {
    return (
      <div className="flex flex-col h-full max-w-lg mx-auto bg-white">
        <Header title={category.name} onBack={() => setCatId(null)} />
        <div className="flex-1 overflow-y-auto px-5 py-4">
          ...
          <div className="space-y-3">
            {category.articles.map((a) => (
              ...
              >
                ...
                <span className="text-xs text-blue-500 font-bold mt-1 inline-block">読む →</span>
              </button>
            ))}
          </div>
        </div>
  ...
  return (
    <div className="flex flex-col h-full max-w-lg mx-auto bg-white">
      <Header title="攻略のコツ" onBack={onBack} />
      <div className="flex-1 overflow-y-auto px-5 py-4">
        <p className="text-sm text-gray-600 mb-4">
          カテゴリを選んで、ネコ・ネズミそれぞれの攻略法を読んでみましょう。
        </p>
          ...
          {TIP_CATEGORIES.map((c) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-gray-400 mt-1">{c.articles.length}記事</p>
              </div>
