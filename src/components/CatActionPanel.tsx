...
export default function CatActionPanel({ currentCatIndex, catPositions, isCpu, isSetup, placedCount = 0 }: Props) {
  if (isCpu) {
    return (
          ...
          return (
            <div
              ...
              className={`flex-1 flex flex-col items-center py-1.5 rounded-xl border-2 text-xs font-bold
                ...
                ${isCurrent ? `${c.bg} ${c.border} ${c.text}` : ''}
                ${i > currentCatIndex ? 'border-gray-200 bg-gray-50 opacity-30' : ''}
              `}
            >
              <span className="text-xl">{isDone ? '✅' : '🐱'}</span>
              <span>ネコ{i + 1}</span>
              <span className="text-gray-400 font-normal">{catPosLabel(catPositions[i])}</span>
            </div>
          );
        })}
      </div>
      <p className="text-xs text-gray-500 text-center">
        ネコ{currentCatIndex + 1}：移動先の通路か捜索するビルをタップ
      </p>
    </div>
  );
}
